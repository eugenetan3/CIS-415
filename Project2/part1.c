//Author: Eugene Tan
//Project 2: Part1.c First portion of the MCP Ghost in a Shell Implementation
...
int processInput(FILE *inputFile, int numPrograms) {
    ...
    while (getline(&buffer, &bufsize, inputFile) != -1) { //Read from input file line by line
        ...
        if (pidArgs[i] == 0) { //Child forked successfully -> call execvp
            ...
            //execvp should not reach here unless an error was encountered
        ...
        printf("Parent has forked child (PID %d)\n",pidArgs[i]);
        free(commandArgs);
        i++;
    }
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 2) { //Check if file argument was passed 
        ...
        if (fp == NULL) { //If file is null throw an error, else proceeed
